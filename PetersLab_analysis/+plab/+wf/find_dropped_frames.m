...
for curr_longframe = reshape(delayed_frames,1,[])
    ...
    % Get the next starting run of frames collected with a normal interval
    ...
    normal_upload_times = ...
        ...
        frame_upload_time_diff(curr_longframe:end) <= normal_interval(2);

    n_normal_frames = 3; % number of normal frames to consider back on track
    next_normal_interval_frame = curr_longframe + ...
        find(movsum(normal_upload_times,[0,n_normal_frames-1]) >= n_normal_frames,1);
