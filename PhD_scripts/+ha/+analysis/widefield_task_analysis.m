...
load('C:\Users\havgana\Desktop\DPhil\packaged_data\task_data_all_animals_08_07.mat'); % wf task
load('C:\Users\havgana\Desktop\DPhil\packaged_data\behaviour_data_all_animals_11_11.mat') % behaviour
load('C:\Users\havgana\Desktop\DPhil\packaged_data\combined_sig_day_all_protocols_07_30_big_stim_and_post_filtered.mat') % sig days

% load master_U
wf_U= plab.wf.load_master_U;

added_time = -2:0.03:2; % for task
added_time_Kernel=  fliplr((-10:52)/30); % kernel
...
% opts.following_names = {'visual_operant_lick_two_stim_static','stim_wheel*'};  % For HA014 for now
...
pre = nanmean(rewarded_stim_v_stacked_data(:,:,workflow_cat==2 & learning_index_animal==0 & is_group_animal==1),3);
post = nanmean(rewarded_stim_v_stacked_data(:,:,workflow_cat==2 & learning_index_animal==1 & is_group_animal==1),3);
...
pre = nanmean(rewarded_stim_v_stacked_data(:,:,workflow_cat==1 & learning_index_animal==0 & widefield_animal_idx==7),3);
post = nanmean(rewarded_stim_v_stacked_data(:,:,workflow_cat==1 & learning_index_animal==1 & widefield_animal_idx==7),3);
...
% define the variables to plot
data_to_plot= rewarded_stim_start_move_v_stacked;
ROI_to_plot= left_mPFC_ROI_mask;
time_to_plot= added_time;
curr_components= n_components;
...
timewindow_last_p2_non_learners = plab.wf.svd2px(wf_U(:,:,1:n_components), baseline_sub_last_p2_non_learners(:,time_window_idx));
...
% Find the corresponding indices in added_time for the time window
time_window_idx = find(added_time_V >= start_time & added_time_V <= end_time);

% % 1×T win vector in seconds
t_for_plot = added_time_V(time_window_idx);

n_components_to_use= n_components; % kernel_n_components or n_components
