...
%% Script for saving passive data

% It goes through recordings and saves the stim onset aligned V and Kernel
...
possible_stim_conditions = {90, -90, 0,22.5,45,67.5};
stim_labels = {'right_stim', 'left_stim', 'center_stim','right_twenty_two_deg_stim','right_fourty_five_deg_stim','right_sixty_seven_deg_stim'};
    ...
    for protocol_idx = 1:length(protocols_to_run)
        ...
        for day_idx = 1:numel(current_recordings_in_use)
            ...
            for rec_idx = idxs_to_run
                ...
                for cond_idx = 1:numel(possible_stim_conditions)
                    stim_position = possible_stim_conditions{cond_idx};
                    ...
                    [kernels,predicted_signals,explained_var,predicted_signals_reduced] = ...
                        ap.regresskernel(wf_V(1:n_components,:),stim_regressor,time_shifts,15);
                    ...
                    if ~contains(workflow_name,'size60') && ~contains(workflow_name,'big_stim')
